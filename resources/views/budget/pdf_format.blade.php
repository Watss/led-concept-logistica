...
<body>
    <table style="width: 100%; border: 0px solid transparent;">
        <tr style="border: 0px solid transparent;">
            <td width="70%" style="border: 0px solid transparent;"><img class="img" src="{{ asset('logo.png') }}"
                    width="80" alt="">
            </td>
            <td width="30%" style="border: 0px solid transparent; vertical-align: bottom;">
                <h5 style="font-size: 10px; font-weight: bold;" class="text-right align-bottom">{{ $date }}</h5>
            </td>
        </tr>
    </table>
    <table class="table border-color-black color-primary mb-5" style="border: 0px solid transparent;">
        <tr style="border: 0px solid transparent;">
            <th class="p-0 pt-2 ml-1" width="80%"
                style="font-size: 10px; border: 0px solid transparent; border-bottom: 5px solid #000000; "><span>LED
                    Concept SPA - Rut: 76.628.935-5 - Dirección: Avda. Tobalaba # 4899, Ñuñoa - Teléfono: 2 2 981 6267
                </span></th>
            <th class="p-0 pt-2 text-right mr-1"
                style="font-size: 10px; border: 0px solid transparent; border-bottom: 5px solid #000000;">
                <span>www.ledconcept.cl</span>
            </th>
        </tr>
    ...
    <table class="table border-color-black">
        ...
        <tbody>
            ...
            @foreach ($budget->detailsBudgets as $key => $detailBudget)
                <tr>
                    <th scope="row" class="color-primary" style="text-align: center; font-weight: 400;">
                        {{ $detailBudget->prefix ?? $key + 1 }} </th>
                    <td style="width: 200px; text-align:center;" class="capitalize">{{ $detailBudget->product->name }}
                    </td>
                    <td>
                        @if (!$detailBudget->product->image)
                        ...
                        @else
                            <img src="{{ asset($detailBudget->product->image) }}" class="image-container">
                    </td>
            @endif
            ...
            <tr style="border-top: 1px solid black; padding: 0px;" class="tr-primaty-style">
                <td colspan="5"
                    style="border:2px solid white; border-right: 1px solid black; padding: 2px; border-top: 1px solid black;">
                </td>
                <td class="color-primary capitalize" style="font-weight: bold; padding: 2px">NETO</td>
                <td style="padding: 2px">@money($budget->neto + $discount)</td>
            </tr>
    ...
    <table class="table border-color-black color-primary mb-5"
        style="border: 0px solid transparent; margin-top:auto;">
